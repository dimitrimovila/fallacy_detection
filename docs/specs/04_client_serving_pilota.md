...
### 2.2 Cache
SQLite in `runs/cache.sqlite`, a table with key = sha256 of (`model_id`, `prompt_version`, text of the rendered prompt, generation parameters, `sample_index`) and value = the serialised `RawResponse`. The cache is consulted before every call; a call already made is not repeated. The cache is never cleared automatically. Changing the prompt changes the key, so a new prompt version generates new calls only for that prompt.
