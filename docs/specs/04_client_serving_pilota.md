...
* `serving/README.md`: runbook in dieci righe. Avvio di vLLM con l'API compatibile OpenAI e l'output strutturato abilitato, con un esempio:
  ...
  ```
  Per Qwen e Gemma le due condizioni si servono con due avvii dello stesso modello, stessa versione di vLLM e stessi pesi. La voce con ragionamento spento si serve senza parser di ragionamento: con il parser acceso e nessun ragionamento, vLLM dalla 0.19.0 in poi può saltare in silenzio il vincolo dello schema JSON. La voce `_think` si serve con il parser di ragionamento di vLLM e con `--max-model-len` abbastanza grande per `max_tokens` più il prompt (16384): senza parser l'output strutturato vincola il JSON fin dal primo token e il modello non può ragionare. L'esecuzione procede per modello, quindi basta riavviare il server fra le due voci.
